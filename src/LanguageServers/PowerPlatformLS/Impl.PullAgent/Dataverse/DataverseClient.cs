namespace Microsoft.PowerPlatformLS.Impl.PullAgent.Dataverse
{
    ...
    internal class DataverseClient : IDataverseClient
    {
        ...
        public virtual async Task UpdateWorkflowAsync(Guid? agentId, WorkflowMetadata? workflowMetadata, CancellationToken cancellationToken)
        {
            ...
            if (!existsInCloud)
            {
                await InsertWorkflowAsync(agentId, workflowMetadata, cancellationToken);
                return;
            }
        ...
        /// <param name="cancellationToken">The cancellation token.</param>
        /// </summary>
        public virtual async Task InsertWorkflowAsync(Guid? agentId, WorkflowMetadata? workflowMetadata, CancellationToken cancellationToken)
        {
            if (workflowMetadata is null)
            {
                throw new ArgumentNullException(nameof(workflowMetadata));
            ...
            var requestBody = CreateWorkflowRequestBody(workflowMetadata);
            requestBody["statecode"] = 0;  // Draft
            requestBody["statuscode"] = 1; // Draft
        ...
        private Dictionary<string, object?> CreateWorkflowRequestBody(WorkflowMetadata workflowMetadata)
        {
            return new Dictionary<string, object?>
            {
                ["name"] = workflowMetadata.Name,
                ["type"] = workflowMetadata.Type,
                ["description"] = workflowMetadata.Description,
                ["subprocess"] = workflowMetadata.Subprocess,
                ["category"] = workflowMetadata.Category,
                ["mode"] = workflowMetadata.Mode,
                ["scope"] = workflowMetadata.Scope,
                ["ondemand"] = workflowMetadata.OnDemand,
                ["triggeroncreate"] = workflowMetadata.TriggerOnCreate,
                ["triggerondelete"] = workflowMetadata.TriggerOnDelete,
                ["asyncautodelete"] = workflowMetadata.AsyncAutodelete,
                ["syncworkflowlogonfailure"] = workflowMetadata.SyncWorkflowLogOnFailure,
                ["statecode"] = workflowMetadata.StateCode,
                ["statuscode"] = workflowMetadata.StatusCode,
                ["runas"] = workflowMetadata.RunAs,
                ["istransacted"] = workflowMetadata.IsTransacted,
                ["introducedversion"] = workflowMetadata.IntroducedVersion,
                ["iscustomizable"] = workflowMetadata.IsCustomizable,
                ["businessprocesstype"] = workflowMetadata.BusinessProcessType,
                ["iscustomprocessingstepallowedforotherpublishers"] = workflowMetadata.IsCustomProcessingStepAllowedForOtherPublishers,
                ["modernflowtype"] = workflowMetadata.ModernFlowType,
                ["primaryentity"] = workflowMetadata.PrimaryEntity,
