    ...
    internal abstract class SyncHandler : IRequestHandler<SyncAgentRequest, SyncAgentResponse, RequestContext>
    {
        ...
        public async Task<SyncAgentResponse> HandleRequestAsync(SyncAgentRequest request, RequestContext context, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var (updatedDefinition, workflowResponse) = await ExecuteAsync(workspace, operationContext, dataverseClient, syncInfo.AgentId, cancellationToken);
                var (_, localChanges) = await _synchronizer.GetLocalChangesAsync(workspace.FolderPath, updatedDefinition, dataverseClient, syncInfo.AgentId, cancellationToken);
