namespace Microsoft.PowerPlatformLS.UnitTests.Impl.PullAgent.Methods
{
    ...
    internal class MockDataverseClient : DataverseClient
    {
        ...
        public override Task InsertWorkflowAsync(Guid? agentId, WorkflowMetadata? workflowMetadata, CancellationToken cancellationToken)
        {
        ...
        public override Task UpdateWorkflowAsync(Guid? agentId, WorkflowMetadata? workflowMetadata, CancellationToken cancellationToken)
        {
            if (workflowMetadata != null)
            {
    ...
    internal class TestWorkspaceSynchronizer : IWorkspaceSynchronizer
    {
        ...
        public Task<(PvaComponentChangeSet, ImmutableArray<Change>)> GetLocalChangesAsync(DirectoryPath workspaceFolder, DefinitionBase workspaceDefinition, CancellationToken cancellationToken)
        {
        ...
        public Task<(PvaComponentChangeSet, ImmutableArray<Change>)> GetRemoteChangesAsync(DirectoryPath workspaceFolder, AuthoringOperationContextBase operationContext, CancellationToken cancellationToken)
        {
            return Task.FromResult((new PvaComponentChangeSet(Enumerable.Empty<BotComponentChange>(), null, "token"), ImmutableArray<Change>.Empty));
        }
        ...
        public Task PushChangesetAsync(DirectoryPath workspaceFolder, AuthoringOperationContextBase operationContext, PvaComponentChangeSet localWorkspaceDefinition, DataverseClient dataverseClient, Guid? agentId, CancellationToken cancellationToken)
            => Task.CompletedTask;

        public Task<WorkspaceSyncInfo> SyncWorkspaceAsync(DirectoryPath workspaceFolder, AuthoringOperationContextBase operationContext, string? changeToken, bool updateWorkspaceDirectory, DataverseClient dataverseClient, Guid? agentId, CancellationToken cancellationToken)
        {
            ReattachCalled = true;
        ...
        public virtual Task UpsertWorkflowForAgentAsync(DirectoryPath workspaceFolder, DataverseClient dataverseClient, Guid? agentId, bool isInsert, CancellationToken cancellationToken)
            => Task.CompletedTask;

        public Task<ImmutableArray<CloudFlowDefinition>> GetWorkflowsAsync(DirectoryPath workspaceFolder, DataverseClient dataverseClient, Guid? agentId, IFileAccessor fileAccessor, CancellationToken cancellationToken)
        {
