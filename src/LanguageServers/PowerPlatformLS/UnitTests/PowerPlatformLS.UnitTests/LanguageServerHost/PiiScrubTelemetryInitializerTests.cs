namespace Microsoft.PowerPlatformLS.UnitTests.LanguageServerHost
{
    ...
    public class PiiScrubTelemetryInitializerTests
    {
        ...
        [InlineData(
            ...
            "File not found: [REDACTED: file-path]")]
        [InlineData(
            @"Error in D:\Projects\vscode-copilotstudio\src\test.cs",
            "Error in [REDACTED: file-path]")]
        [InlineData(
            "Authenticated as user@contoso.com successfully",
            "Authenticated as [REDACTED: email] successfully")]
        [InlineData(
            "No PII here, just a normal message",
            "No PII here, just a normal message")]
        [InlineData(
            @"Multiple paths: C:\Users\admin\file.txt and D:\data\secret.json",
            "Multiple paths: [REDACTED: file-path] and [REDACTED: file-path]")]
        [InlineData(
            "Agent at /home/developer/agents/MyBot/agent.mcs.yml",
            "Agent at [REDACTED: file-path]")]
        [InlineData(
            "Workspace: /Users/john/Projects/copilot-studio/agent/",
            "Workspace: [REDACTED: file-path]")]
        [InlineData(
            "Temp file /tmp/mcs-sync-12345/output.json failed",
            "Temp file [REDACTED: file-path] failed")]
        [InlineData(
            "Destination path 'c:/Users/gngangomtiem/Projects/MCSAgents/Test Agent/' already exists",
            "Destination path '[REDACTED: file-path]' already exists")]
        ...
        public void Initialize_Scrubs_TraceTelemetry_Message_And_Properties()
        {
            ...
            Assert.Equal("Error at [REDACTED: file-path]", trace.Message);
            Assert.Equal("[REDACTED: email] failed", trace.Properties["FormattedMessage"]);
        ...
        public void Initialize_Scrubs_ExceptionTelemetry_Properties()
        {
            ...
            Assert.Equal("[REDACTED: email] error", ex.Properties["{OriginalFormat}"]);
        }
        ...
        [Fact]
        public void Initialize_Scrubs_ErrorMessage_Property()
        {
            var initializer = new PiiScrubTelemetryInitializer();
            var trace = new TraceTelemetry("operation failed");
            trace.Properties["ErrorMessage"] = @"File not found: C:\Users\john\agent.mcs.yml";
            trace.Properties["errorMessage"] = @"Path error: C:\Users\dev\project";

            initializer.Initialize(trace);

            Assert.Equal("File not found: [REDACTED: file-path]", trace.Properties["ErrorMessage"]);
            Assert.Equal("Path error: [REDACTED: file-path]", trace.Properties["errorMessage"]);
        }

        [Theory]
        [InlineData(
            "Agent 191e2a0a-5390-f111-8077-000d3a199a69 not found",
            "Agent [REDACTED: id] not found")]
        [InlineData(
            "No GUIDs here",
            "No GUIDs here")]
        [InlineData(
            "Bot 191E2A0A-5390-F111-8077-000D3A199A69 in env 00000000-0000-0000-0000-000000000001",
            "Bot [REDACTED: id] in env [REDACTED: id]")]
        ...
        public void ScrubMessage_Handles_Mixed_Tagged_And_Untagged_PII()
        {
            ...
            Assert.Equal("[REDACTED: email] also [REDACTED: email] in message", result);
        }
        ...
        [InlineData(
            ...
            "[REDACTED: email] loaded")]
        [InlineData(
            @"<pii type=""EMAIL ADDRESS"" encoded=""true"">user@contoso.com</pii> > Developer: Loaded 0 environment(s)",
            "[REDACTED: email] > Developer: Loaded 0 environment(s)")]
        [InlineData(
        ...
        [Fact]
        public void Initialize_Scrubs_ExceptionTelemetry_Message()
        {
            var initializer = new PiiScrubTelemetryInitializer();
            var exception = new System.IO.FileNotFoundException(@"Could not find file 'C:\Users\john\agents\MyAgent\topics\greeting.mcs.yml'.");
            var telemetry = new ExceptionTelemetry(exception);

            initializer.Initialize(telemetry);

            Assert.Equal("Could not find file '[REDACTED: file-path]'.", telemetry.Message);
        }

        [Fact]
        public void Initialize_Scrubs_ExceptionTelemetry_DetailMessages()
        {
            var initializer = new PiiScrubTelemetryInitializer();
            var exception = new System.IO.FileNotFoundException(@"Could not find file 'C:\Users\john\secret.txt'.");
            var telemetry = new ExceptionTelemetry(exception);

            initializer.Initialize(telemetry);

            // ExceptionDetailsInfoList is populated from the exception; verify the detail message is scrubbed
            Assert.NotEmpty(telemetry.ExceptionDetailsInfoList);
            Assert.Equal("Could not find file '[REDACTED: file-path]'.", telemetry.ExceptionDetailsInfoList[0].Message);
        }
    }
}
