...
namespace Microsoft.CommonLanguageServerProtocol.Framework
{
    ...
    {
        private static readonly MethodInfo ProcessQueueCoreAsyncMethodInfo = typeof(RequestExecutionQueue<TRequestContext>)
                ...
                .GroupBy(m => m.MethodName))
            {
                ...
                foreach (var metadata in methodGroup)
                {
                    languages.Add(metadata.Language, new(() =>
                    {
                        ...
                        var method = ProcessQueueCoreAsyncMethodInfo.MakeGenericMethod(requestType, responseType);
        ...
        private (RequestHandlerMetadata Metadata, IMethodHandler Handler, MethodInfo MethodInfo) GetHandlerForRequest(IQueueItem<TRequestContext> work, string language)
        {
            ...
            _logger.LogWarning($"Method '{work.MethodName}' is not implemented for language '{language}'. Cancelling request.");
