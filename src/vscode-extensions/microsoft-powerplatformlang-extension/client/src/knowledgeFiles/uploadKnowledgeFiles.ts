...
import { getDataverseBotHandler, getFilePaths } from './syncUtils';
...
export async function uploadKnowledgeFiles(ws: CopilotStudioWorkspace): Promise<void> {
  ...
  const { filesDir, trackPath } = getFilePaths(workspaceUri);
  ...
  const allFiles = (await fs.readdir(filesDir).catch(() => [])).filter(f => !f.endsWith('.mcs.yml'));
  const toUpload: { file: string; fullPath: string; localModifiedOn: number; size: number; botId: string; schema: string }[] = [];
  ...
  for (const file of allFiles) {
    ...
    if (!prevNow || (remoteModifiedOn && remoteModifiedOn !== prevNow.remoteModifiedOn)) {
      ...
      if (!botRec) {
        ...
        const result = await resolveConflict(file, full, remoteContent, await isTextFile(file));
    ...
    if (needToUpload) {
      const botId = botRec
        ? botRec.id
        : await botHandler.getBotComponentId(file, schema, syncInfo.agentId);
      toUpload.push({ file, fullPath: full, localModifiedOn: fileStat.mtimeMs, size: fileStat.size, botId, schema });
    }
  ...
  await vscode.window.withProgress({
    ...
    for (let i = 0; i < toUpload.length; i++) {
      ...
      const { file, fullPath, localModifiedOn, size, botId, schema } = toUpload[i];
      let success = false;
