...
import { getDataverseBotHandler, getFilePaths } from '../../knowledgeFiles/syncUtils';
...
suite('uploadKnowledgeFiles', () => {
    ...
    setup(async () => {
        ...
        (getFilePaths as any) = (_ws: vscode.Uri) => ({ filesDir: workspaceDir, trackPath });

        (getDataverseBotHandler as any) = async () => ({
            listWsComponentMetadata: async () => [
                { id: '1', schemaName: 'bot.file.file1', modifiedOn: Date.now(), filename: 'file1.txt', sizeInBytes: 7 },
                { id: '2', schemaName: 'bot.file.file2', modifiedOn: Date.now(), filename: 'file2.txt', sizeInBytes: 7 }
            ],
