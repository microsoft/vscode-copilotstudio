...
suite('TextDocumentContentProvider Integration', function () {
	...
	test('Local cache provider returns content', async () => {
		...
		await assert.rejects(
			async () => { await vscode.workspace.openTextDocument(missingFileTestUri); },
			(err: any) => {
                assert.ok(
                    err?.message.startsWith(
                        `cannot open ${missingFileTestUri}`
                    ),
                    `Unexpected error message: ${err?.message}`
                );
				return true;	// signal assert.rejects that the error matched
			}
		);
