...
import { getEnvironmentByIdAsync, findEnvironmentInListAsync, EnvironmentLookup } from '../clients/bapClient';
...
export async function tryRepairAgentManagementEndpoint(syncInfo: AgentSyncInfo, workspaceUri: string, lookups?: EnvironmentLookup[]): Promise<boolean> {
  ...
  const agentManagementUrl = await resolveAgentManagementUrl(syncInfo.environmentId, syncInfo.accountInfo, lookups ?? [getEnvironmentByIdAsync, findEnvironmentInListAsync]);
