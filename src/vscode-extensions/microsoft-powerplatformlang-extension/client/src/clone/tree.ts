    ...
    return new Promise(async (resolve) => {
      if (element === undefined) {
        ...
        try {
          ...
          logger.logError(`[TreeView] Failed to load ${skuItem.sku} environments: ${e?.message || e}`, 'environments');
          logger.logFeatureEvent({
            feature: 'environments',
            operation: 'loadEnvironments',
            outcome: 'failure',
        ...
        try {
          ...
          logger.logError(`[TreeView] Failed to load agents for ${envItem.environment.displayName}: ${e?.message || e}`, 'environments');
          logger.logFeatureEvent({
            feature: 'environments',
            operation: 'loadAgents',
            outcome: 'failure',
            errorMessage: e?.message || String(e),
            environmentId: envItem.environment.environmentId,
          });
