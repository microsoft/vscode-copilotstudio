...
export class botComponentHandler {
  ...
  public async listWsComponentMetadata(syncInfo: AgentSyncInfo): Promise<WsComponentMetadata[]> {
    ...
    const filter = `startswith(schemaname,'${botPrefix}')`;
    const query = [
  ...
  public async createBotComponent(name: string, schemaName: string, agentId: string): Promise<string> {
    const postData = JSON.stringify({
      name,
      description: `Knowledge source for ${name}`,
      componenttype: 14,
      schemaname: schemaName,
      'parentbotid@odata.bind': `/bots(${agentId})`
    });
